...
(function(){
  ...
  function buildRanksTable(schemes) {
    ...
    schemes.forEach(function(s){
      var sc=computeScore(s),nm=s.schemeName||"";
  ...
  function buildRiskTable(schemes) {
    var tH="padding:10px 12px;font-size:10px;color:var(--text-muted);font-weight:700;white-space:nowrap;";
    var mCols=[['alpha_1m','Alpha 1M'],['alpha_1y','Alpha 1Y'],['alpha_3y','Alpha 3Y'],['beta_1m','Beta 1M'],['beta_1y','Beta 1Y'],['beta_3y','Beta 3Y'],['sharpe_1m','Sharpe 1M'],['sharpe_1y','Sharpe 1Y'],['sharpe_3y','Sharpe 3Y'],['sortino_1m','Sortino 1M'],['sortino_1y','Sortino 1Y'],['sortino_3y','Sortino 3Y'],['treynor_1m','Treynor 1M'],['treynor_1y','Treynor 1Y'],['treynor_3y','Treynor 3Y'],['stdDev_1m','StdDev 1M'],['stdDev_1y','StdDev 1Y'],['stdDev_3y','StdDev 3Y']];
    var h='<table style="width:100%;border-collapse:collapse;font-size:12px;min-width:1600px;"><thead><tr style="background:var(--bg-header);border-bottom:2px solid var(--line);">';
    h+='<th style="'+tH+'width:28px;"></th><th style="'+tH+'text-align:left;">Scheme name</th>';
    mCols.forEach(function(mc){h+='<th style="'+tH+'text-align:right;">'+mc[1]+'</th>';});
    ...
    schemes.forEach(function(s){
      var m=_metrics[s.id]||{},nm=s.schemeName||"";
      var sid=(s.id||'').replace(/'/g,"\\\\");
      h+='<tr class="mf-table-row" data-cat="'+(s.category||'')+'" data-amc="'+(s.parentAmc||s.amc||'Other')+'" style="border-bottom:1px solid var(--line);cursor:pointer;" onclick="openMfDetailFromTable(\''+sid+'\')">';
      h+='<td style="padding:10px 8px;width:28px;"><span style="color:var(--text-muted);font-size:14px;">\u2606</span></td>';
      h+='<td style="padding:10px 12px;font-weight:700;font-size:12px;color:var(--text-primary);max-width:320px;"><div style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'+nm+'</div><div style="font-size:10px;color:var(--text-muted);">'+(s.category||'')+'</div></td>';
      mCols.forEach(function(mc){
        var v=m[mc[0]];
        if(v!=null){
          if(mc[0].indexOf("alpha")!==-1){var sg=v>0?"+":"";var cl=v>=0?"#00B386":"#EB5B56";h+='<td style="padding:10px 12px;text-align:right;font-size:12px;color:'+cl+';font-weight:600;">'+sg+(v*100).toFixed(2)+'%</td>';}
          else if(mc[0].indexOf("stdDev")!==-1){h+='<td style="padding:10px 12px;text-align:right;font-size:12px;">'+(v*100).toFixed(2)+'%</td>';}
      ...
      h+='<td style="padding:10px 12px;text-align:right;font-size:12px;color:var(--text-primary);font-weight:700;">'+(s.aumCr!=null?"\u20b9"+Number(s.aumCr).toLocaleString("en-IN",{maximumFractionDigits:0})+" Cr":"-")+'</td>';
      h+='<td style="padding:10px 12px;text-align:right;font-size:12px;">'+(s.expenseRatio!=null?s.expenseRatio.toFixed(2)+"%":"-")+'</td>';
      var inv=s.investorCount!=null?(s.investorCount>10000?(s.investorCount/100000).toFixed(2)+"L":s.investorCount.toLocaleString("en-IN")):"-";
      h+='<td style="padding:10px 12px;text-align:right;font-size:12px;font-weight:600;">'+inv+'</td></tr>';
    });
    return h+'</tbody></table>';
  }
  
  window.setMfViewTab = function(tab) {
    _currentTab = tab;
    document.querySelectorAll('.mf-view-tab').forEach(function(btn){
      var active = btn.getAttribute('data-tab') === tab;
      btn.style.background = active ? '#00B386' : 'var(--bg-input)';
      btn.style.color = active ? '#fff' : 'var(--text-muted)';
      btn.style.borderColor = active ? '#00B386' : 'var(--line)';
    });
    ...
    if (!wrap) return;
    if (tab === 'Summary') {
      renderMfGrid(_lastSchemes);
    } else if (tab === 'Ranks') {
      wrap.innerHTML = buildRanksTable(_lastSchemes);
    } else if (tab === 'Risk') {
      wrap.innerHTML = buildRiskTable(_lastSchemes);
    }
  };
  
  var origRender = window.renderMfGrid;
  if (origRender) {
  ...
  var observer = new MutationObserver(function() {
    var grid = document.getElementById('mfSchemesGrid');
    if (!grid) return;
    if (document.querySelector('.mf-view-tab')) return;
    var catTabsDiv = grid.querySelector('div[style*="flex-wrap"]');
    if (catTabsDiv && catTabsDiv.parentNode === grid) {
      ...
      ['Summary','Ranks','Risk vs Reward'].forEach(function(tb){
        ...
        var active = _currentTab === key;
        var btn = document.createElement('button');
        btn.className = 'mf-view-tab';
        btn.setAttribute('data-tab', key);
        btn.textContent = tb;
        btn.style.cssText = 'padding:7px 16px;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;border:1px solid '+(active?'#00B386':'var(--line)')+';background:'+(active?'#00B386':'var(--bg-input)')+';color:'+(active?'#fff':'var(--text-muted)')+';transition:all .15s;font-family:inherit;';
        btn.onclick = function(){ window.setMfViewTab(key); };
        tabsDiv.appendChild(btn);
      });
      catTabsDiv.parentNode.insertBefore(tabsDiv, catTabsDiv.nextSibling);
    }
  });
  
  var gridEl = document.getElementById('mfSchemesGrid');
