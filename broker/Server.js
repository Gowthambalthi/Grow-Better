...
app.get('/api/instruments/watchlist', async (req, res) => {
  try {
    const defaultData = [
      { symbol: 'NIFTY', name: 'NIFTY 50', quote: { price: 24190.80, close: 24287.65, change: -96.85, changePct: -0.40 } },
      { symbol: 'BANKNIFTY', name: 'BANK NIFTY', quote: { price: 57300.45, close: 57497.80, change: -197.35, changePct: -0.34 } },
      { symbol: 'SENSEX', name: 'SENSEX', quote: { price: 77328.20, close: 77728.16, change: -399.96, changePct: -0.51 } },
      { symbol: 'GIFTNIFTY', name: 'GIFT NIFTY', quote: { price: 24200.00, close: 24287.65, change: -87.65, changePct: -0.36 } },
      { symbol: 'GOLD', name: 'MCX GOLD', quote: { price: 155404.00, close: 154754.00, change: 650.00, changePct: 0.42 } },
      { symbol: 'SILVER', name: 'MCX SILVER', quote: { price: 272500.00, close: 271000.00, change: 1500.00, changePct: 0.55 } }
    ];

    let liveSuccess = false;
    const session = brokers.angelone?.session || req.app.get('angelSession');
    if (session) {
      try {
        const tokensByExch = { 'NSE': ['99926000', '99926009'], 'BSE': ['99919000'] };
        const fetched = await angelMarketQuote.getQuote(session, tokensByExch, 'FULL');
        for (const q of fetched) {
          const ltp = Number(q.ltp || q.last_price || 0);
          const close = Number(q.close || q.closePrice || ltp);
          const chg = ltp - close;
          const chgPct = close > 0 ? (chg / close) * 100 : 0;

          if ((q.symbolToken === '99926000' || q.symboltoken === '99926000') && ltp > 0) {
            defaultData[0].quote = { price: ltp, close, change: chg, changePct: chgPct };
            liveSuccess = true;
          } else if ((q.symbolToken === '99926009' || q.symboltoken === '99926009') && ltp > 0) {
            defaultData[1].quote = { price: ltp, close, change: chg, changePct: chgPct };
            liveSuccess = true;
          } else if ((q.symbolToken === '99919000' || q.symboltoken === '99919000') && ltp > 0) {
            defaultData[2].quote = { price: ltp, close, change: chg, changePct: chgPct };
            liveSuccess = true;
          }
        }
      } catch (e) {
        console.error('[watchlist] Angel One live quote API note:', e.message);
      }
    }
    ...
    if (!liveSuccess) {
      try {
        const yMap = [
          { idx: 0, ySym: '%5ENSEI' },
          { idx: 1, ySym: '%5ENSEBANK' },
          { idx: 2, ySym: '%5EBSESN' },
        ];
        for (const item of yMap) {
          const yUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${item.ySym}?interval=1m&range=1d`;
          const yRes = await axios.get(yUrl, { headers: { 'User-Agent': 'Mozilla/5.0' }, timeout: 3500 });
          ...
          if (meta && meta.regularMarketPrice != null) {
            ...
            const chg = ltp - close;
            const chgPct = close > 0 ? (chg / close) * 100 : 0;
            defaultData[item.idx].quote = { price: ltp, close, change: chg, changePct: chgPct };
          }
        }
      } catch (e) {}
    }
